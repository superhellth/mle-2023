...
def setup(self):
    ...
    self.round_counter3 = 0
    self.crop_size = 5 #only odd numbers
    self.steps1 = 0
    self.step_experience1 = 0
    self.lock = 0
    self.newRound = False
...
def act(self, game_state: dict) -> str:
    if game_state['step'] == 1: #Get old overall exploration vs exploitation mode
        ...
        self.round_counter3 +=1
        self.steps1 = 1
        self.lock = 1
        self.newRound = True
        self.randomized_round = 0.8 +  random.uniform(0, 0.2) #for this round we will use qtable intensively or rarely
    """
    ...
    if best_action == None:
        state = (tuple(tuple(row) for row in crop),direction_advice1,bomb_possible)
        ...
        if self.train:
            if self.steps1 < self.step_experience1: #Wenn weniger als 10 Schritte orientiere dich an die Table mit unterschiedlicher Wkeit [0,...,1], danach exploration
                    for action in possible_actions:
                        qvalue = self.qtable.get((state,action),float('-inf'))
                        if qvalue > best_qvalue:
                            best_action = action
        ...
        if best_qvalue == float('-inf'): #Once exploitation is reached, we go full exploitation modus
            #weights = [1, 1, 1, 1, 1, 1]
            #subset_weights = [weights[ACTIONS.index(action)] for action in possible_actions]

            best_action =random.choice(possible_actions) #random.choices(possible_actions, subset_weights)[0]#random.choice(['UP', 'RIGHT', 'DOWN', 'LEFT','WAIT','BOMB'])

            #print("Random move done! from possible actions")
      #self.movement_history.append(best_action)

    '''if self.step_experience1 > 800:
        self.step_experience1 = 300'''
    if self.round_counter2 % 1000 == 0:
        if self.lock:
            self.step_experience1 += 1
            print(self.step_experience1)
            self.lock = 0
    '''if self.round_counter2 % (8000+1) == 0:
        if self.lock:
            self.step_experience1 = 0
            self.lock = 0'''
    self.steps1+=1
